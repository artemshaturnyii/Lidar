import socket
import struct
import time
import math

# Константы протокола
CMD_PW_ON = b'\xA5\x30'
CMD_START = b'\xA5\x20'
CMD_STOP  = b'\xA5\x25'
M1_HEADER = 0x54
NODES_PER_PACK = 12
PACKET_FMT = '<BBHH' + 'HB' * NODES_PER_PACK + 'HHB'

# Таблица CRC8 (полная)
CRC8_TABLE = [
    
]

def crc8(data: bytes) -> int:
    crc = 0
    for b in data:
        crc = CRC8_TABLE[crc ^ b]
    return crc

def connect_and_get_two_scans(host='192.168.0.7', port=25168):
    """Получает два полных скана для охвата 0-360°"""
    
    print(f"Connecting to lidar {host}:{port}...")
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.connect((host, port))
    
    # Включение питания
    print("Power on...")
    sock.send(CMD_PW_ON)
    time.sleep(2)
    
    # Запуск сканирования
    print("Start scanning...")
    sock.send(CMD_START)
    time.sleep(1)
    
    print("Collecting two scans to cover full 360°...")
    print("=" * 60)
    
    # Собираем все точки из двух сканов
    all_points = []
    last_angle = None
    scan_count = 0
    
    try:
        while scan_count < 2:  # Собираем два скана
            # Ищем заголовок 0x54
            while True:
                header_byte = sock.recv(1)
                if not header_byte:
                    return
                if header_byte[0] == M1_HEADER:
                    break
            
            # Читаем остальные 46 байт
            remaining_data = bytearray()
            while len(remaining_data) < 46:
                chunk = sock.recv(46 - len(remaining_data))
                if not chunk:
                    return
                remaining_data.extend(chunk)
            
            full_packet = bytes([M1_HEADER]) + remaining_data
            
            # Проверяем CRC
            if crc8(full_packet[:-1]) != full_packet[-1]:
                print("CRC check failed, skipping packet")
                continue
                
            # Распаковываем пакет
            try:
                fields = struct.unpack(PACKET_FMT, full_packet)
                start_angle_raw = fields[3]
                start_angle = start_angle_raw * 0.01
                
                # Извлекаем узлы (точки)
                nodes = []
                for i in range(NODES_PER_PACK):
                    distance = fields[4 + i*2]
                    intensity = fields[4 + i*2 + 1]
                    nodes.append((distance, intensity))
                
                # Конечный угол
                end_angle_raw = fields[4 + 2*NODES_PER_PACK]
                end_angle = end_angle_raw * 0.01
                
                # Вычисляем шаг угла между точками
                if end_angle < start_angle:
                    step = (end_angle + 360 - start_angle) / (len(nodes) - 1)
                else:
                    step = (end_angle - start_angle) / (len(nodes) - 1)
                
                # Обрабатываем каждую точку
                for i, (distance, intensity) in enumerate(nodes):
                    angle = (start_angle + step * i) % 360
                    
                    # Проверяем переход через 0° - это означает завершение скана
                    if last_angle is not None and angle < last_angle - 300:  # Большой скачок назад
                        scan_count += 1
                        print(f"Scan {scan_count} complete!")
                        if scan_count >= 2:
                            break
                    
                    all_points.append({
                        'angle': angle,
                        'distance': distance,
                        'intensity': intensity
                    })
                    last_angle = angle
                    
                if scan_count >= 2:
                    break
                    
            except Exception as e:
                print(f"Packet parsing error: {e}")
                continue
                
    except KeyboardInterrupt:
        print(f"\nStopped by user. Collected {len(all_points)} points.")
    finally:
        # Остановка сканирования
        try:
            sock.send(CMD_STOP)
        except:
            pass
        sock.close()
    
    return all_points

def normalize_angle(angle):
    """Нормализует угол в диапазон 0-360"""
    return angle % 360

def sort_and_normalize_points(points):
    """Сортирует точки по углу и нормализует углы 0-360°"""
    # Нормализуем углы
    for point in points:
        point['angle'] = normalize_angle(point['angle'])
    
    # Сортируем по углу
    return sorted(points, key=lambda x: x['angle'])

def filter_unique_angles(points, tolerance=0.1):
    """Фильтрует точки, оставляя только уникальные углы с заданной точностью"""
    filtered_points = []
    last_angle = -1000  # Начальное значение, которое точно меньше любого угла
    
    for point in points:
        current_angle = point['angle']
        # Если угол значительно отличается от предыдущего, добавляем точку
        if abs(current_angle - last_angle) > tolerance:
            filtered_points.append(point)
            last_angle = current_angle
    
    return filtered_points

def main():
    points = connect_and_get_two_scans()
    
    if not points:
        print("No points collected")
        return
    
    print(f"\nTotal points collected: {len(points)}")
    
    # Сортируем и нормализуем точки
    sorted_points = sort_and_normalize_points(points)
    
    # Фильтруем повторяющиеся углы
    filtered_points = filter_unique_angles(sorted_points)
    
    print(f"Points after filtering: {len(filtered_points)}")
    
    # Находим минимальный и максимальный угол
    angles = [p['angle'] for p in filtered_points]
    if angles:
        min_angle = min(angles)
        max_angle = max(angles)
        print(f"Angle coverage: {min_angle:.1f}° - {max_angle:.1f}°")
    
    print(f"\n=== Full Scan Results ({len(filtered_points)} points) ===")
    print("Points sorted by angle (0° to 360°):")
    print("-" * 50)
    
    for i, point in enumerate(filtered_points):
        print(f"point {i:3d}, angle:{point['angle']:8.3f}, range: {point['distance']:6.1f}, intensity:{point['intensity']:3.0f}")
    
    # Показываем статистику
    if filtered_points:
        distances = [p['distance'] for p in filtered_points]
        intensities = [p['intensity'] for p in filtered_points]
        
        print(f"\n=== Statistics ===")
        print(f"Angle range: {min(angles):.1f}° - {max(angles):.1f}°")
        print(f"Distance range: {min(distances):.1f}mm - {max(distances):.1f}mm")
        print(f"Intensity range: {min(intensities):.0f} - {max(intensities):.0f}")
        print(f"Average points per degree: {len(filtered_points)/360:.2f}")

if __name__ == "__main__":
    main()
